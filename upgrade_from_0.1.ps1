...
if ($SourcePyprojectText -notmatch 'version\s*=\s*"0\.3\.0"') {
    throw "Source directory does not look like AIH-Contexture 0.3.0: $SourceDir"
}
...
$BackupRoot = Join-Path (Split-Path -Parent $TargetDir) ("AIH-Contexture-backup-before-0.3-" + $Timestamp)
