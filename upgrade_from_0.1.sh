...
if ! grep -q 'version = "0.5.0"' "$SOURCE_DIR/pyproject.toml"; then
  echo "[upgrade] Source directory does not look like AIH-Contexture 0.5.0: $SOURCE_DIR" >&2
  exit 1
fi
...
BACKUP_ROOT="$(dirname "$TARGET_DIR")/AIH-Contexture-backup-before-0.5-$TIMESTAMP"

DIR_ITEMS=(.github .streamlit aih_contexture assets data examples signatures static tests tools)
FILE_ITEMS=(
  pyproject.toml poetry.lock requirements.txt README.md CHANGELOG.md LICENSE MODEL_LICENSE NOTICE CLA.md
  VERSION_0.5_RELEASE_REPORT.md chunk_convert.py contexture_app.py contexture_server.py convert.py
  convert_single.py extraction_app.py install.bat install.sh install.command start.bat start.sh start.command
  upgrade_from_0.1.bat upgrade_from_0.1.ps1 upgrade_from_0.1.sh
...
log "Protected in target: .venv venv .git .claude .env configs output uploads conversion_results debug_data temp .cache"
...
if [[ "$SKIP_PIP_INSTALL" != "1" ]]; then
  if [[ -x "$TARGET_DIR/.venv/Scripts/python.exe" ]]; then
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
      log "Would run: $TARGET_DIR/.venv/bin/python -m pip install -e $TARGET_DIR --no-deps"
    else
      log "Refreshing editable install without dependencies."
      "$TARGET_DIR/.venv/bin/python" -m pip install -e "$TARGET_DIR" --no-deps
    fi
  else
