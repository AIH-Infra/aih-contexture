...
def _build_default_registry() -> LayoutBackendRegistry:
    ...
    registry.register(
        LayoutBackendSpec(
            ...
            display_name="MinerU Pipeline Sidecar",
            capabilities=BackendCapabilities(
                name="mineru_pp_doclayout_v2",
                kind="layout",
                display_name="MinerU Pipeline Sidecar",
                ...
                supports_formulas=True,
                notes=(
                    "Optional MinerU CLI adapter. It runs the MinerU pipeline to produce *_middle.json, "
                    "then reuses Contexture external layout normalization before rendering. "
                    "MinerU pipeline may still run its own OCR/table/formula sub-steps."
                ),
            ),
        )
    )
