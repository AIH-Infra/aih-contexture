...
def _build_default_registry() -> VlmBackendRegistry:
    ...
    registry.register(
        VlmBackendSpec(
            name="surya2",
            display_name="Surya 2",
            capabilities=BackendCapabilities(
                name="surya2",
                kind="vlm",
                display_name="Surya 2",
                requires_service=True,
                optional_dependency="surya-ocr-2-service",
                supports_bbox=True,
                supports_confidence=False,
                supports_vertical_text=True,
                supports_footnote=True,
                ...
                languages=("multi",),
                notes=(
                    "Implemented VLM specialized backend for Surya 2. It uses the "
                    "official layout JSON prompt followed by block HTML OCR prompts, "
                    "then adapts the normalized blocks into Contexture Middle."
                ),
            ),
        )
    )
