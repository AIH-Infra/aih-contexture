...
def _build_default_registry() -> OcrBackendRegistry:
    ...
    registry.register(
        OcrBackendSpec(
            name="chrome_screenai",
            display_name="Chrome ScreenAI",
            capabilities=BackendCapabilities(
                name="chrome_screenai",
                kind="ocr",
                display_name="Chrome ScreenAI",
                optional_dependency="chrome-screenai",
                supports_bbox=False,
                supports_confidence=False,
                supports_vertical_text=True,
                languages=("multi",),
                notes=(
                    "Special OCR backend that first generates a searchable PDF with Chrome ScreenAI, "
                    "then re-enters the pipeline through embedded PDF text with the selected layout backend."
                ),
            ),
        )
    )
    ...
    registry.register(
        OcrBackendSpec(
            ...
            capabilities=BackendCapabilities(
                ...
                languages=("multi",),
                notes=(
                    "Pipeline OCR adapter that runs Surya 2 block HTML OCR on existing "
                    "layout block crops through an OpenAI-compatible vision service."
                ),
            ),
        )
    )
    ...
    registry.register(
        OcrBackendSpec(
            ...
            capabilities=BackendCapabilities(
                ...
                languages=("multi", "cjk"),
                notes=(
                    "Optional OCR adapter using MinerU's internal PytorchPaddleOCR "
                    "(models/OCR/paddleocr_torch). It can be paired with any layout backend."
                ),
            ),
        )
    )
