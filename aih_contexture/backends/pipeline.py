...
def create_layout_builder(
    *,
    ...
    if layout_backend == "mineru_vl_layout":
        from aih_contexture.builders.mineru_vl_layout import MineruVLLayoutBuilder

        logger.info("[PdfConverter] Using MinerU-VL Layout Detection backend")
        return MineruVLLayoutBuilder(config=config)

    if layout_backend == "surya2_layout":
        from aih_contexture.builders.surya2_layout import Surya2LayoutBuilder

        logger.info("[PdfConverter] Using Surya 2 Layout JSON backend")
        return Surya2LayoutBuilder(config=config)
...
def create_ocr_builder(
    *,
    ...
    if ocr_backend == "surya2_ocr":
        from aih_contexture.builders.surya2_ocr import Surya2OcrBuilder

        logger.warning("[PdfConverter] Using Surya 2 block OCR backend")
        return Surya2OcrBuilder(config=config)

    if ocr_backend == "mineru_pytorch_paddle_ocr":
        from aih_contexture.builders.mineru_ocr import MineruPytorchPaddleOcrBuilder

        logger.warning("[PdfConverter] Using MinerU PyTorch/Paddle OCR backend")
        return MineruPytorchPaddleOcrBuilder(config=config)
