...
class PaddleOCRVLOcrBuilder(BaseBuilder):
    """Pipeline block OCR builder backed by PaddleOCR-VL VLRecognition."""
    ...
    def __init__(
        ...
        *,
        service: OcrPaddleOCRVLService | None = None,
    ):
        super().__init__(config)
        self.config = config or {}
        service_config = dict(self.config)
        service_config.setdefault("paddleocr_vl_backend_name", "paddleocr_vl_ocr")
        self.service = service or OcrPaddleOCRVLService(service_config)
        self._ocr_helper = OcrBuilder(recognition_model=None, config=self.config)
        self.crop_padding_px = int(self.config.get("paddleocr_vl_crop_padding_px", 4) or 0)
        self.crop_padding_frac = float(self.config.get("paddleocr_vl_crop_padding_frac", 0.02) or 0.0)
        self.block_concurrency = max(1, int(self.config.get("paddleocr_vl_block_concurrency", 4) or 4))
        self.api_key = self.config.get("paddleocr_vl_api_key") or self.config.get("ocr_api_key")
    ...
    ) -> list[PaddleOCRVLRegionCrop]:
        crops: list[PaddleOCRVLRegionCrop] = []
        for page in pages:
            ...
            for block in page.structure_blocks(document):
                ...
                crops.append(
                    PaddleOCRVLRegionCrop(
                        ...
                        prompt_label=self._prompt_label_for_block(block),
                    )
                )
        return crops

    async def _recognize_crops(self, crops: list[PaddleOCRVLRegionCrop]) -> list[Any]:
        sem = asyncio.Semaphore(self.block_concurrency)
        async with aiohttp.ClientSession() as session:
            async def run_one(crop: PaddleOCRVLRegionCrop) -> Any:
                async with sem:
                    return await self.service.recognize_image_async(
                        session,
                        crop.image,
                        api_key=self.api_key,
                        prompt_label=crop.prompt_label,
                    )
    ...
    def _write_result(self, document: Document, crop: PaddleOCRVLRegionCrop, result: dict[str, Any]) -> None:
        ...
        try:
            block.set_internal_metadata("ocr_backend", "paddleocr_vl_ocr")
            block.set_internal_metadata("paddleocr_vl_prompt_label", crop.prompt_label)
        except Exception:
            pass

    def _line_items_from_result(
        self,
        crop: PaddleOCRVLRegionCrop,
        result: dict[str, Any],
        ...
        if isinstance(blocks, list):
            for block in blocks:
                ...
                items.append((text, polygon or crop.block.polygon))
        if items:
            return items
    ...
    def _local_bbox_to_page_polygon(self, crop: PaddleOCRVLRegionCrop, local_bbox: Any) -> PolygonBox | None:
        if not (
    ...
    def _span(self, SpanClass, page: PageGroup, text: str, polygon: PolygonBox):
        span = SpanClass(
            text=text.rstrip("\n") + " ",
            formats=["plain"],
        ...
        try:
            span.set_internal_metadata("ocr_backend", "paddleocr_vl_ocr")
        except Exception:
            pass
        return span

    @staticmethod
    def _prompt_label_for_block(block: Any) -> str:
        block_type = getattr(block, "block_type", None)
        if block_type == BlockTypes.Table:
            return "table"
        if block_type == BlockTypes.Equation:
            return "formula"
        return "ocr"
