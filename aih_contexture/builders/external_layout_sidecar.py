    ...
    for position, block in enumerate(sorted(sidecar_page.blocks, key=lambda item: (item.order, item.id))):
        ...
        boxes.append(
            LayoutBox(
                ...
                polygon=polygon,
                metadata=_layout_metadata(block),
            )
        )
...
def _layout_metadata(block: MiddleBlock) -> dict[str, Any]:
    attrs = dict(block.attrs or {})
    provenance_raw_label = None
    for item in block.provenance or []:
        if item.raw_label:
            provenance_raw_label = item.raw_label
            break
    raw_label = attrs.get("raw_label") or provenance_raw_label or block.type
    metadata = {
        "raw_label": raw_label,
        "backend_label": raw_label,
        "label_source": attrs.get("label_source") or "backend_native",
    }
    if attrs.get("native_layout_backend"):
        metadata["native_layout_backend"] = attrs["native_layout_backend"]
    if attrs.get("marginal_source"):
        metadata["marginal_source"] = attrs["marginal_source"]
    if attrs.get("block_source"):
        metadata["block_source"] = attrs["block_source"]
    raw = attrs.get("raw")
    if isinstance(raw, dict):
        block_source = raw.get("block_source")
        if block_source:
            metadata["block_source"] = block_source
    return metadata
