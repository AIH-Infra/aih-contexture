...
from aih_contexture.config.vlm_model_presets import default_quant, default_version, resolve_vlm_model
...
def chrome_screenai_preprocess_fields(values: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "ocr_preprocess_backend": values.get("ocr_preprocess_backend", "none"),
        "chrome_screenai_light": bool(values.get("chrome_screenai_light", False)),
        "chrome_preprocess_mode": values.get("chrome_preprocess_mode", "native"),
        "chrome_workers": values.get("chrome_workers", values.get("ocr_concurrency", 2)),
        "chrome_chunk_pages": values.get("chrome_chunk_pages", values.get("ocr_batch_size", 4)),
        "chrome_emit_searchable_pdf": bool(values.get("chrome_emit_searchable_pdf", False)),
        "chrome_rasterize_dpi": values.get("chrome_rasterize_dpi", 144),
        "chrome_model_dir": values.get("chrome_model_dir"),
    }
...
def mineru_vl_layout_fields(values: Mapping[str, Any]) -> dict[str, Any]:
    version = values.get("mineru_vl_version", default_version("mineru_vl"))
    quant = values.get("mineru_vl_quant", default_quant("mineru_vl"))
    model = values.get("mineru_vl_model") or values.get("ocr_model")
    if model is None:
        model = resolve_vlm_model("mineru_vl", version=version, quant=quant)
    endpoint = values.get("mineru_vl_endpoint") or values.get("ocr_endpoint") or values.get("openai_base_url")
    api_key = values.get("mineru_vl_api_key")
    if api_key is None:
        api_key = values.get("ocr_api_key", values.get("openai_api_key"))
    return {
        "mineru_vl_endpoint": endpoint,
        "mineru_vl_api_key": api_key,
        "mineru_vl_api_style": values.get("mineru_vl_api_style", values.get("ocr_api_style", "lmstudio-native")),
        "mineru_vl_version": version,
        "mineru_vl_quant": quant,
        "mineru_vl_model": model,
        "mineru_vl_layout_image_size": values.get("mineru_vl_layout_image_size", (1036, 1036)),
        "mineru_vl_layout_timeout": values.get("mineru_vl_layout_timeout", values.get("ocr_timeout", 120)),
        "mineru_vl_layout_max_tokens": values.get("mineru_vl_layout_max_tokens", 4096),
        "mineru_vl_layout_batch_size": values.get("mineru_vl_layout_batch_size", values.get("mineru_vl_layout_concurrency", 1)),
        "mineru_vl_layout_concurrency": values.get("mineru_vl_layout_concurrency", values.get("mineru_vl_request_concurrency", 1)),
        "mineru_vl_request_concurrency": values.get("mineru_vl_request_concurrency", values.get("mineru_vl_layout_concurrency", 1)),
        "mineru_vl_image_quality": values.get("mineru_vl_image_quality", values.get("ocr_image_quality", 90)),
    }


def surya2_vlm_fields(values: Mapping[str, Any], *, layout_only: bool = False, pipeline_ocr: bool = False) -> dict[str, Any]:
    version = values.get("surya2_version", default_version("surya2"))
    api_style = values.get("surya2_api_style", values.get("ocr_api_style", "openai"))
    endpoint = values.get("surya2_endpoint")
    if endpoint is None:
        endpoint = values.get("ocr_endpoint", values.get("openai_base_url"))
    endpoint = _openai_vlm_endpoint(endpoint, api_style=api_style)
    model = values.get("surya2_model")
    if model is None:
        model = values.get("ocr_model", values.get("openai_model"))
    if model is None:
        model = resolve_vlm_model("surya2", version=version)
    api_key = values.get("surya2_api_key")
    if api_key is None:
        api_key = values.get("ocr_api_key", values.get("openai_api_key"))
    default_request_concurrency = 6 if layout_only else values.get("ocr_concurrency", 6)
    default_block_concurrency = 4 if pipeline_ocr else values.get("surya2_block_concurrency", 4)
    return {
        ...
        "surya2_model": model,
        "surya2_layout_timeout": values.get("surya2_layout_timeout", values.get("ocr_timeout", 120)),
        "surya2_layout_max_tokens": values.get("surya2_layout_max_tokens", 4096),
        "surya2_layout_batch_size": values.get("surya2_layout_batch_size", values.get("surya2_layout_concurrency", default_request_concurrency)),
        "surya2_layout_concurrency": values.get("surya2_layout_concurrency", values.get("surya2_request_concurrency", default_request_concurrency)),
        "surya2_request_concurrency": values.get("surya2_request_concurrency", default_request_concurrency),
        "surya2_block_concurrency": values.get("surya2_block_concurrency", default_block_concurrency),
        "surya2_image_format": values.get("surya2_image_format", "PNG"),
        "surya2_image_quality": values.get("surya2_image_quality", values.get("ocr_image_quality", 90)),
        "surya2_crop_padding_px": values.get("surya2_crop_padding_px", 4),
        "surya2_crop_padding_frac": values.get("surya2_crop_padding_frac", 0.02),
    }


def mineru_ocr_fields(values: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "mineru_ocr_python": values.get("mineru_ocr_python") or values.get("mineru_layout_python") or values.get("mineru_python"),
        "mineru_ocr_lang": values.get("mineru_ocr_lang", values.get("paddle_ocr_lang", "ch")),
        "mineru_ocr_device": values.get("mineru_ocr_device"),
        "mineru_ocr_timeout": values.get("mineru_ocr_timeout", values.get("mineru_timeout", 3600)),
        "mineru_ocr_det_db_box_thresh": values.get("mineru_ocr_det_db_box_thresh", 0.3),
        "mineru_ocr_det_db_unclip_ratio": values.get("mineru_ocr_det_db_unclip_ratio", 1.8),
        "mineru_ocr_enable_merge_det_boxes": values.get("mineru_ocr_enable_merge_det_boxes", True),
    }
...
def _openai_vlm_endpoint(endpoint: Any, *, api_style: Any) -> str:
    style = str(api_style or "openai").strip().lower()
    if style == "openai-compatible":
        style = "openai"
    if not endpoint:
