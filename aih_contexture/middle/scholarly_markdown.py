...
def _render_block(
    ...
    *,
    ...
    if block_type not in {"Equation", "Table", "Code", "Form"}:
    ...
    if block_type == "MarginalNote":
        if not include_margin_comments:
            return [text, ""]
        return _render_margin_block(block, text, page_width=_positive_float(page.get("width")), anchor_start=anchor_start, anchor_end=anchor_end)
