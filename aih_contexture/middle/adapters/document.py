    ...
    return MiddlePage(
        ...
        provenance=[
            MiddleProvenance(
                ...
                stage="document",
                model=layout_model,
                source=getattr(document, "filepath", None),
            )
        ],
    )
...
def _block_to_middle(
    ...
    *,
    ...
    raw_label = getattr(block_type, "name", None) or str(block_type or "")
    canonical_type = normalize_block_type(raw_label)
    ...
    if raw_label != canonical_type:
        attrs["raw_block_type"] = raw_label
