...
def _extract_blocks(page_data: dict[str, Any], *, block_source: str = "auto") -> list[dict[str, Any]]:
    ...
    if normalized_source == "auto":
        mineru_blocks = _auto_mineru_blocks_with_discarded_auxiliaries(page_data)
        if mineru_blocks is not None:
...
def _auto_mineru_blocks_with_discarded_auxiliaries(page_data: dict[str, Any]) -> list[dict[str, Any]] | None:
    """Keep MinerU page auxiliaries that are scholarly signal, not noise.

    MinerU writes headers, page numbers, side notes, and page footnotes to
    discarded_blocks. For humanities material, aside_text/page numbers/footnotes
    are citation structure and must survive the default import path.
    """

    if not isinstance(page_data.get("discarded_blocks"), list):
        return None
    ...
    blocks = [
        dict(item, block_source=item.get("block_source", primary_key))
        for item in page_data.get(primary_key, [])
        if isinstance(item, dict)
    ]
    for item in page_data.get("discarded_blocks", []):
        if not isinstance(item, dict):
            continue
        raw_label = str(_first_value(item, ("label", "type", "layout_label", "category", "block_type", "block_label")) or "")
        normalized = raw_label.strip().lower().replace("-", "_").replace(" ", "_")
        if normalized in {"aside_text", "page_aside_text", "page_number", "number", "page_footnote", "footnote"}:
            blocks.append(dict(item, block_source=item.get("block_source", "discarded_blocks")))
    return blocks
