...
def ocr_direct_outputs_to_middle_document(
    ...
    *,
    ...
    elif normalized_backend in {"paddleocr_vl", "mineru_vl", "mineru_vl_page", "surya2", "surya_ocr_2"}:
...
def _churro_element_to_block(
    ...
    *,
    ...
    if isinstance(element.get("inline_marks"), list):
        attrs["inline_marks"] = element.get("inline_marks")
    provenance = MiddleProvenance(
        backend=backend,
    ...
    if text:
        spans.append(
            MiddleSpan(
                text=text,
                attrs={
                    "source": "official_xml_json_text",
                    **({"inline_marks": element.get("inline_marks")} if isinstance(element.get("inline_marks"), list) else {}),
                },
