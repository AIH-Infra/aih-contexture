...
def layout_result_to_middle_page(
    ...
    *,
    ...
    for order, (_, box) in enumerate(ordered_boxes):
        box_metadata = dict(getattr(box, "metadata", {}) or {})
        box_label = getattr(box, "label", None)
        native_raw_label = box_metadata.get("raw_label") or box_metadata.get("backend_label")
        native_block_type = normalize_block_type(str(native_raw_label)) if native_raw_label else None
        if native_block_type and native_block_type != "ComplexRegion":
            raw_label = native_raw_label
            block_type = native_block_type
        else:
            raw_label = box_label
        ...
        for key in (
            ...
            if key in box_metadata:
                attrs[key] = box_metadata[key]
        if native_raw_label and str(native_raw_label) != str(box_label):
            attrs["raw_block_type"] = box_label
