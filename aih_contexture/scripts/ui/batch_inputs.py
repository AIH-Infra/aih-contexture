...
def input_file_objects(upload_mode: str, uploaded_files: Iterable, ctx: dict | None = None) -> list[tuple[object, str]]:
    if upload_mode == UPLOAD_MODE:
        if ctx is not None and ctx.get("_staged_upload_files") is not None:
            return list(ctx["_staged_upload_files"])
    ...
    for file_content, file_name in file_objects:
        try:
            pdf_source = (
                ...
                if upload_mode == UPLOAD_MODE and isinstance(file_content, bytes)
                else file_content
            )
