...
def _prefixed_message(file_name: str, event: dict[str, Any], fallback: str) -> str:
    ...
    *,
    ...
    def callback(event: dict[str, Any]) -> None:
        ...
        if event_name == "pages_discovered":
            ...
            progress["stage"] = "预处理"
            progress["message"] = _prefixed_message(
                file_name,
                event,
                f"{file_name} · 共 {total_pages} 页，正在分批渲染",
            )
            ...
            progress["message"] = _prefixed_message(
                file_name,
                event,
                f"{file_name} · 正在渲染第 {start_page}-{end_page} 页",
            )
            ...
            if loaded_pages is None:
                ...
                file_state["total_pages"] = total_pages
            progress["stage"] = "处理中"
            progress["message"] = _prefixed_message(file_name, event, f"已载入 {file_name}")
            ...
            progress["message"] = _prefixed_message(file_name, event, f"{file_name} · 第 {page_num} 页")
            ...
            progress["stage"] = "保存可搜索PDF"
            progress["current_page"] = None
            progress["message"] = _prefixed_message(
                file_name,
                event,
                f"{file_name} · 正在生成可搜索 PDF",
            )

        elif event_name == "postprocess":
            progress["stage"] = "整理输出"
            progress["current_page"] = None
            progress["message"] = _prefixed_message(file_name, event, f"{file_name} 正在整理结果")

        elif event_name == "file_done":
            progress["stage"] = "保存结果"
            progress["current_page"] = None
            progress["message"] = _prefixed_message(file_name, event, f"{file_name} 已完成，正在保存")
