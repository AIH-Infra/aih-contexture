...
PIPELINE_UI_VALUE_KEYS = (
    ...
    "chrome_screenai_light",
    "chrome_preprocess_mode",
    "chrome_workers",
    "chrome_chunk_pages",
    "chrome_emit_searchable_pdf",
    ...
    "mineru_layout_runtime_mode",
    "sidecar_runtime_mode",
    "mineru_vl_endpoint",
    "mineru_vl_api_key",
    "mineru_vl_api_style",
    "mineru_vl_version",
    ...
    "mineru_vl_layout_timeout",
    "mineru_vl_layout_max_tokens",
    "mineru_vl_layout_batch_size",
    "mineru_vl_layout_concurrency",
    "mineru_vl_request_concurrency",
    "mineru_vl_image_quality",
    "surya2_endpoint",
    "surya2_api_key",
    "surya2_api_style",
    "surya2_version",
    "surya2_model",
    "surya2_layout_timeout",
    "surya2_layout_max_tokens",
    "surya2_layout_batch_size",
    "surya2_layout_concurrency",
    "surya2_request_concurrency",
    "surya2_block_concurrency",
    "surya2_image_format",
    "surya2_image_quality",
    "surya2_crop_padding_px",
    "surya2_crop_padding_frac",
    "mineru_ocr_python",
    "mineru_ocr_lang",
    "mineru_ocr_device",
    "mineru_ocr_timeout",
    "mineru_ocr_det_db_box_thresh",
    "mineru_ocr_det_db_unclip_ratio",
    "mineru_ocr_enable_merge_det_boxes",
    ...
    "native_marginalia_enabled",
    "heuristic_marginal_detection_enabled",
...
def pipeline_config_for_page_range(page_range: tuple[int, int], values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ocr_preprocess_backend = _resolve_pipeline_ocr_preprocess_backend(
        str(ocr_backend),
        values.get("ocr_preprocess_backend", "none"),
    )
    ...
    config.update(pipeline_preprocess_backend_config(values))
...
def pipeline_layout_backend_config(layout_backend: str, values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if layout_backend == "mineru_pp_doclayout_v2_direct":
        config = mineru_direct_layout_fields(values)
        config["mineru_layout_runtime_mode"] = values.get("mineru_layout_runtime_mode") or "auto"
        if values.get("sidecar_runtime_mode"):
            config["sidecar_runtime_mode"] = values.get("sidecar_runtime_mode")
        return config

    if layout_backend == "mineru_vl_layout":
        return mineru_vl_layout_fields(values)

    if layout_backend == "surya2_layout":
        return surya2_vlm_fields(values, layout_only=True)
...
def pipeline_ocr_backend_config(ocr_backend: str, values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if ocr_backend == "surya2_ocr":
        return surya2_vlm_fields(values, pipeline_ocr=True)

    if ocr_backend == "mineru_pytorch_paddle_ocr":
        return mineru_ocr_fields(values)
...
def pipeline_preprocess_backend_config(values: Mapping[str, Any]) -> dict[str, Any]:
    backend = _resolve_pipeline_ocr_preprocess_backend(
        str(values.get("ocr_backend", "surya") or "surya"),
        values.get("ocr_preprocess_backend", "none"),
    )
    config = chrome_screenai_preprocess_fields(values)
...
def pipeline_markdown_processor_config(values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "equation_output_mode": values.get("equation_output_mode", "humanities_safe"),
        "list_enabled": values.get("list_enabled", True),
        "footnote_enabled": footnote_enabled,
        "superscript_policy": "footnote_safe" if footnote_enabled else "suppress_footnote_like",
...
def pipeline_memory_config(
    ...
    explicit_build_highres = values.get("build_highres_images")
    if explicit_build_highres is not None:
        return {
            "build_highres_images": bool(explicit_build_highres),
            "image_extraction_mode": values.get(
                "image_extraction_mode",
                "highres" if explicit_build_highres else "lowres",
            ),
        }

    if conversion_mode == "pipeline" and ocr_backend == "none":
        return {
...
def pipeline_scholarly_config(values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    legacy_marginal_detection = bool(values.get("enable_marginal_detection", False))
    native_marginalia_enabled = bool(values.get("native_marginalia_enabled", legacy_marginal_detection))
    heuristic_marginal_detection_enabled = bool(
        values.get("heuristic_marginal_detection_enabled", legacy_marginal_detection)
    )
    ...
    marginal_output_mode = normalize_marginal_output_mode(
        values.get("marginal_output_mode"),
    ...
    if enable_marginal_detection:
        config.update({
            ...
            "native_marginalia_enabled": native_marginalia_enabled,
            "heuristic_marginal_detection_enabled": heuristic_marginal_detection_enabled,
        ...
        config["native_marginalia_enabled"] = False
        config["heuristic_marginal_detection_enabled"] = False
        config["marginal_output_mode"] = marginal_output_mode
