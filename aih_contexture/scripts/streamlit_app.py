...
**MinerU Pipeline Sidecar** - 兼容/诊断路径
- 调用 MinerU CLI 完整 pipeline 生成 sidecar
- 用于兼容、对照和诊断，不是纯版面检测路径
- 可能触发 MinerU 内部 OCR、表格、公式等阶段
""",
...
MODE_STATE_RULES = {
    ...
    "pipeline": {
        "prefixes": ("pipeline_", "llm_", "calamari_", "vlm_layout_", "vlm_ocr_", "tesseract_", "ocr_crop_", "markdown_postprocess_"),
        "include": {
...
with st.sidebar:
    ...
    markdown_postprocess_llm_api_key = "lm-studio"
    ...
    if conversion_mode == "vlm_specialized":
        ...
        # OCR 后端选择
        specialized_ocr_backend_options = ["chandra", "churro", "paddleocr_vl", "mineru_vl"]
...
- 输出保留模型原始结果，先归一化 MinerU 协议，再标准化为 Contexture Middle
        ...
        with st.expander("🔌 API 配置", expanded=True):
            default_ocr_api_style = "openai" if ocr_backend == "churro" else "lmstudio-native"
        ...
        with st.expander("高级选项", expanded=False):
            ...
            if ocr_concurrency_mode == "serial_file":
                ...
                st.caption(f"串行模式：逐个文件处理，每 {ocr_concurrency} 页后等待 {ocr_batch_rest} 秒。")
            else:
                # 单页多文件批次模式
                col1, col2 = st.columns(2)
                with col1:
                ...
                st.caption(f"批次模式：每批 {ocr_file_batch_size} 个文件，完成后等待 {ocr_file_batch_rest} 秒。")
            ...
            with col2:
                ...
                ocr_timeout = st.number_input(
                    ...
                    help="单个 API 请求的超时时间。MinerU-VL 经 LM Studio 两阶段解析时通常需要至少 600 秒，避免客户端超时后服务端仍继续生成。",
                    key="ocr_timeout"
                )
...
- 本地推理通常需要降低页面并发
    ...
    if conversion_mode == "vlm_generalized":
        ...
        elif layout_backend == "mineru_pp_doclayout_v2":
            mineru_layout_settings = render_mineru_layout_settings(
                st,
                description=LAYOUT_BACKEND_DESCRIPTIONS["mineru_pp_doclayout_v2"],
            )
            mineru_command = mineru_layout_settings["mineru_command"]
            mineru_output_dir = mineru_layout_settings["mineru_output_dir"]
            mineru_backend = mineru_layout_settings["mineru_backend"]
            mineru_method = mineru_layout_settings["mineru_method"]
            mineru_lang = mineru_layout_settings["mineru_lang"]
            mineru_api_url = mineru_layout_settings["mineru_api_url"]
            mineru_server_url = mineru_layout_settings["mineru_server_url"]
            mineru_timeout = mineru_layout_settings["mineru_timeout"]
            mineru_extra_args = mineru_layout_settings["mineru_extra_args"]
