...
from aih_contexture.services.layout_service import (
    BaseLayoutService,
    LayoutBox,
    LayoutResult,
...
class DocLayoutYoloService(BaseLayoutService):
    """
    DocLayout-YOLO 版面识别服务。
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        logger.info(
            f"[DocLayoutYoloService] Init: base_url={self.yolo_base_url}, "
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            for endpoint in endpoints:
                try:
                    ...
                    if resp.status_code == 200:
                        logger.info(f"[DocLayoutYoloService] Health check passed: {endpoint}")
                        return True
                except Exception:
                    continue
            ...
            logger.warning(f"[DocLayoutYoloService] Health check failed: {e}")
            return False
    ...
    def get_available_models(self) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"[DocLayoutYoloService] Failed to get models: {e}")
            return []
    ...
    def detect_layout(
        ...
        for img in images:
            try:
                ...
                logger.error(f"[DocLayoutYoloService] Detection failed: {e}")
                # 返回空结果
                w, h = img.size
    ...
    def _detect_single_image(self, img: Image.Image) -> LayoutResult:
        ...
        for attempt in range(self.yolo_max_retries + 1):
            try:
                ...
                last_error = e
                logger.warning(
                    f"[DocLayoutYoloService] Attempt {attempt + 1} failed: {e}"
                )
        ...
        # 所有重试失败
        logger.error(f"[DocLayoutYoloService] All retries failed: {last_error}")
        w, h = original_size
        return LayoutResult(
            image_bbox=[0, 0, w, h],
            bboxes=[],
