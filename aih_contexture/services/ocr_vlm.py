...
class VlmOcrService(BaseService):
    openai_base_url: Annotated[str, "Base URL for OpenAI-compatible endpoint."] = "https://dashscope.aliyuncs.com/compatible-mode/v1"
    openai_model: Annotated[str, "Model name."] = "qwen-vl-max-2025-01-25"
    openai_api_key: Annotated[str, "API key (LM Studio accepts any)."] = ""
    ...
    def __init__(self, config=None):
        ...
        if self.key_rotator.get_key_count() > 1:
            logger.info(f"[VlmOcrService] Using {self.key_rotator.get_key_count()} API keys with rotation")

        logger.info(f"[VlmOcrService] Init: base_url={self.openai_base_url}, model={self.openai_model}")
        logger.info(f"[VlmOcrService] Image: format={self.openai_image_format}, max_dim={self.max_image_dimension}")
        logger.info(f"[VlmOcrService] Mode: vlm_response_mode={self.vlm_response_mode}, openai_use_stop={self.openai_use_stop}")
    ...
    def get_client(self, api_key: Optional[str] = None) -> openai.OpenAI:
        ...
        return openai.OpenAI(base_url=self.openai_base_url, api_key=key or "lm-studio")
    ...
    def __call__(
        ...
        for stage_idx, hard in enumerate(quality_stages):
            ...
            for i in range(1, api_tries_total + 1):
                try:
                    # Get current key and create client
                    ...
                    if _is_suspicious(raw_text):
                        logger.warning(
                            "[VlmOcrService] Suspicious output; stage=%d/%d hard=%s",
                            stage_idx + 1,
                            len(quality_stages),
                            str(hard),
                        )
                    ...
                    logger.warning(f"[VlmOcrService] Retryable error: {e}")
                    if i < api_tries_total:
                        # Rotate to next key on failure
                        next_key = self.key_rotator.mark_failure_and_rotate()
                        if self.key_rotator.get_key_count() > 1:
                            logger.info(f"[VlmOcrService] Rotating to next API key (attempt {i+1}/{api_tries_total})")
                        time.sleep(2)
                        continue
                    last_result = {"lines": []}
                except Exception as e:
                    logger.error(f"[VlmOcrService] Error: {e}")
                    # Rotate key on generic errors too
                    if i < api_tries_total:
                        next_key = self.key_rotator.mark_failure_and_rotate()
                        if self.key_rotator.get_key_count() > 1:
                            logger.info(f"[VlmOcrService] Rotating to next API key after error")
                        time.sleep(2)
                        continue
                    last_result = {"lines": []}
                    break
    ...
    def call_with_key(
        ...
        """
        使用指定的API Key执行OCR (用于并发场景)

        这个方法专门用于OrderedConcurrentExecutor，允许每个并发任务使用不同的Key。
        ...
        try:
            ...
            # 恢复原始key
            self.openai_api_key = original_key

# Backward compatibility alias (deprecated)
# TODO: Remove this alias in a future version
