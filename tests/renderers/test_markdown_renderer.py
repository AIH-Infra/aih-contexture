...
from aih_contexture.renderers.markdown import MarkdownFormatter, MarkdownRenderer
...
def test_markdown_formatter_does_not_turn_page_separator_into_list_item():
    formatter = MarkdownFormatter()

    markdown = formatter.format("{0}\n---\nBody\n-item")
...
@pytest.mark.config({"page_range": [1], "paginate_output": True})
def test_markdown_renderer_pagination_uses_physical_page_interval_anchor(pdf_document):
    renderer = MarkdownRenderer({"paginate_output": True})
    md = renderer(pdf_document).markdown
