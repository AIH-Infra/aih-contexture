...
import marker.builders.vlm_ocr
...
# ==================== 页面配置 ====================
st.set_page_config(page_title="兰台·PageAnchor：面向人文学科与档案馆的文献级PDF结构化引擎", layout="wide")
st.title("📄 兰台·PageAnchor：面向人文学科与档案馆的文献级PDF结构化引擎")
st.markdown("基于 Marker 的 PDF 文档结构化提取（支持多种 OCR 后端）")
...
with st.sidebar:
    ...
    with col1:
        out_dir = st.text_input("输出文件夹", value=st.session_state.output_dir, label_visibility="collapsed")
    ...
    if conversion_mode == "vlm_direct":
        st.info(
            "🚀 **VLM Direct 模式**\n\n"
            "- ✅ 完全跳过 Surya（Layout + OCR）\n"
            "- ✅ 纯 VLM 处理整页\n"
            "- ✅ 异步并发（提速 5-10 倍）\n"
            "- ✅ 最简单、最快速\n\n"
            "**性能**：25页文档约 1-2 分钟（传统模式需要 3-6 分钟）"
        )
    else:
        st.success(
            "🔧 **传统模式**\n\n"
            "使用 Marker 的完整 pipeline，支持：\n"
            "- 版面识别（Surya/VLM/YOLO）\n"
            "- OCR（Surya/VLM/Calamari）\n"
            "- 结构化处理\n"
            "- LLM 增强"
        )
    ...
    # ==================== 2. 文件选择 ====================
    st.subheader("📄 文件选择")
    upload_mode = st.radio("选择模式", ["上传文件", "选择文件夹"], index=0, horizontal=True)
    ...
    if upload_mode == "上传文件":
        ...
        folder_path = st.text_input("文件夹路径", value="", label_visibility="collapsed", placeholder="输入文件夹路径...")
        uploaded_files = []
        if folder_path:
    ...
    with st.expander("页码锚点设置", expanded=False):
        st.caption("配置页码识别和显示格式（适用于所有转换模式）")

        enable_page_anchors = st.checkbox(
            "启用页码锚点",
            value=True,
            help="在每页开头添加页码锚点，便于引用和跳转"
        )
        ...
        if enable_page_anchors:
            ...
            page_anchor_template = st.selectbox(
                ...
                format_func=lambda x: {
                    "{n}": "0-based 页序（0, 1, 2...）",
                    "{n1}": "1-based 页序（0, 1, 2...）",
                    "{printed}": "仅印刷页码（XII, 308...）",
                    "{printed-or-n1}": "优先印刷页码，否则 1-based（推荐人文学科）",
                    "{printed} ({n1})": "印刷页码 + 1-based 组合（XII (1), 308 (2)...）"
                }.get(x, x),
                help="选择页码显示格式"
            )
            ...
            page_anchor_start = st.number_input(
                ...
                max_value=1,
                value=0,
                help="1-based 页序的起始值：0 表示第一页显示为 0，1 表示第一页显示为 1"
            )
            ...
            if conversion_mode == "vlm_direct":
                page_anchor_position = st.radio(
                    ...
                    horizontal=True,
                    help="锚点插入位置（仅 VLM Direct 模式支持）"
                )
            else:
                page_anchor_position = "before"  # Pipeline 模式固定为 before
            ...
            if extract_printed_pages and conversion_mode == "pipeline":
                ...
                with col2:
                    ...
                    printed_page_footer_start = st.slider(
                        ...
                        value=0.83,
                        step=0.01,
                        help="页面底部开始位置（0.83 = 底部 17%）"
                    )
            else:
                # 默认值（VLM Direct 或未启用印刷页码）
    ...
    if conversion_mode == "vlm_direct":
        ...
        vlm_direct_api_key = st.text_input(
            "API Key",
        ...
        vlm_direct_max_concurrent = st.slider(
            ...
            max_value=20,
            value=5,
            help="同时处理的页面数。推荐：OpenAI 5-10，本地模型 1-2",
            key="vlm_direct_max_concurrent"
        )

        st.info(
            f"💡 **并发数建议**\n\n"
            f"当前设置：{vlm_direct_max_concurrent} 并发\n\n"
            f"- OpenAI 付费：5-10\n"
            f"- OpenAI 免费：2-3\n"
            f"- 通义千问：5-10\n"
            f"- Claude：3-5\n"
        ...
        # 图像配置
        with st.expander("🎨 图像配置", expanded=False):
            col1, col2 = st.columns(2)
            with col1:
            ...
            with col2:
                vlm_direct_jpeg_quality = st.number_input(
                    "JPEG 质量 (1-100)",
                    min_value=1,
                    max_value=100,
                    value=90,
        ...
        # API 调用配置
        with st.expander("⚙️ API 调用配置", expanded=False):
            col1, col2 = st.columns(2)
            with col1:
                ...
                vlm_direct_max_tokens = st.number_input(
                    "最大输出 token 数",
                    min_value=512,
                    max_value=16384,
                    value=8192,
                    key="vlm_direct_max_tokens"
                )
            with col2:
        ...
        # 提示词配置
        with st.expander("📝 提示词配置", expanded=False):
        ...
        vlm_direct_extract_printed_pages = extract_printed_pages
            ...
            if layout_backend == "surya":
                ...
                st.success("🎯 **DocLayout-YOLO 版面识别**\n\n通过 Docker 服务进行 YOLO 版面检测。")

                yolo_base_url = st.text_input(
                    "YOLO 服务地址",
                    value=yolo_base_url,
                    help="DocLayout-YOLO Docker 服务地址",
                )
                ...
                with st.expander("YOLO 高级设置", expanded=False):
                    yolo_model = st.text_input("模型名称", value=yolo_model)
            ...
            ocr_backend = st.selectbox(
                "选择 OCR 引擎",
                options=["surya", "calamari", "vlm", "none"],
                index=0,
                format_func=lambda x: {
                    "none": "🚫 禁用 OCR（使用 PDF 内嵌文本）",
            ...
            if ocr_backend == "none":
                ...
                st.success("🔮 **Surya OCR**\n\n内置深度学习 OCR 引擎，支持多语言。")
                with st.expander("Surya 配置", expanded=False):
                    ocr_batch_size = st.slider("OCR 批次大小", 1, 64, int(ocr_batch_size), help="每批处理的图像数量")
                    force_ocr = st.checkbox("强制 OCR", value=False, help="即使有文本层也强制进行 OCR")

                # LLM 增强配置移到独立区块

            # 在 "config_dict = build_config_dict(config_params)" 之后添加
            elif ocr_backend == "calamari":
                st.success("📜 **Calamari OCR**\n\n历史文档专用 OCR 引擎。")

                # 服务状态检查
                calamari_base_url = st.text_input("API 地址", value=calamari_base_url, help="Calamari Docker 服务地址")
                ...
                if available_models:
                    ...
                    calamari_model = st.selectbox("OCR 模型", options=available_models, index=default_idx)
                    if cached_models:
                ...
                with st.expander("🎛️ 高级参数", expanded=False):
                    vlm_prompt = st.text_area("自定义 Prompt", value=vlm_prompt, height=100)
            ...
            if use_llm:
                with st.expander("LLM 配置", expanded=True):
                    ...
                    help="选择 LLM API 提供商。Ollama 可用于本地模型或 OpenAI 兼容的远程 API。"
                    )
    ...
    if start_button:
        ...
        if conversion_mode == "vlm_direct":
            ...
            vlm_direct_config = {
                ...
                "vlm_direct_max_tokens": vlm_direct_max_tokens,
                "vlm_direct_max_retries": vlm_direct_max_retries,
                # 页码锚点配置
                "vlm_direct_enable_page_anchors": vlm_direct_enable_page_anchors,
                "vlm_direct_page_anchor_template": vlm_direct_page_anchor_template,
                "vlm_direct_page_anchor_start": vlm_direct_page_anchor_start,
                "vlm_direct_page_anchor_wrapper": "{{{}}}",  # 标准格式
            ...
            for file_idx, file_obj in enumerate(uploaded_files):
                ...
                try:
                    st.info(f"🚀 使用 VLM Direct 模式（{vlm_direct_max_concurrent} 并发）")
                    ...
                    output_path = os.path.join(st.session_state.output_dir, f"{fname_base}.md")
                    ...
                    st.success(f"✅ 转换完成！输出：{output_path}")
                    st.write(f"📊 总字符数：{len(markdown)}")

                    # 显示预览
                    with st.expander("📄 Markdown 预览", expanded=False):
                        st.markdown(markdown[:2000] + ("..." if len(markdown) > 2000 else ""))
                    ...
                    if upload_mode == "上传文件" and os.path.exists(file_path):
                        os.unlink(file_path)
        ...
        for file_idx, file_obj in enumerate(uploaded_files):
            ...
            try:
                ...
                for bidx, (start, end) in enumerate(page_ranges):
                    ...
                    # 页码配置
                    config_params.update({
                        "page_anchor_template": page_anchor_template,
                        "page_anchor_start": page_anchor_start,
                    })
                    ...
                    if printed_page_enabled:
                        config_params.update({
                            ...
                            "printed_page_header_y_frac": printed_page_header_y_frac,
                            "printed_page_footer_y_frac": printed_page_footer_y_frac,
                        })
                    else:
                        config_params.update({
                    ...
                    if use_llm:
                        config_params.update({
                            ...
                            "llm_base_url": llm_base_url if llm_provider in ["azure", "ollama"] else None,
                            "llm_model": llm_model,
                    ...
                    st.write(f"- 当前时间: `{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}`")
                    st.write("=" * 60)
                    ...
                    if torch.cuda.is_available():
                        ...
                        st.info(f"🔧 GPU cache cleared. Free memory: {torch.cuda.get_device_properties(0).total_memory / 1024**3:.1f} GB")
                ...
                def write_text_with_meta_and_images(main_ext: str, main_text: str, images_dict: dict, meta: dict):
                    ...
                    # ========== 诊断日志：文件写入 ==========
                    file_exists_before = os.path.exists(main_path)
                    # ======================================
                    ...
                    file_mtime = datetime.fromtimestamp(os.path.getmtime(main_path))
                    st.write(f"📝 **文件写入**: `{os.path.basename(main_path)}`")
                    st.write(f"   - 文件大小: {file_size:,} bytes")
                    st.write(f"   - 修改时间: {file_mtime.strftime('%Y-%m-%d %H:%M:%S')}")
                    st.write(f"   - 写入前存在: {'是' if file_exists_before else '否'}")
                    st.write(f"   - 内容预览: {main_text[:100]}...")
                    # ==========================================
                    ...
                    from marker.output import convert_if_not_rgb
                ...
                st.write("=" * 60)
                st.write(f"⏱️ **总处理时间**: {total_elapsed:.2f}秒")
                st.write("=" * 60)
                st.write("")
                st.write("📁 **生成的文件**:")
